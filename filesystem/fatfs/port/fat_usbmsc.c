...
extern unsigned int g_ulMSCInstance;
    ...
    BYTE bValue)                /* Physical drive number (0) */
{
    ...
    if (USBHMSCDriveReady(g_ulMSCInstance)) return(FR_NOT_READY);
    ...
    BYTE count)             /* Sector count (1..255) */
{
    ...
    /* READ BLOCK */
    if (USBHMSCBlockRead(g_ulMSCInstance, sector, buff, count) == 0)
        return RES_OK;
    ...
    BYTE count)             /* Sector count (1..255) */
{
    ...
    /* WRITE BLOCK */
    if(USBHMSCBlockWrite(g_ulMSCInstance, sector, (unsigned char *)buff,
                         count) == 0)
        return RES_OK;
