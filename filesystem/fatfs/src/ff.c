...
static
FRESULT dir_read (
	FATDIR *dj			/* Pointer to the directory object that pointing the entry to be read */
)
{
	FRESULT res;
	BYTE c, *dir;
...
)
{
	...
	*p = 0;
	...
	if (fno->lfname) {
		...
		WCHAR w, *lfn;
		// alivehex@gmail.com
		// with -O3 complie by MDK, fno-lfsize alway is 0, no long file name
		fno->lfsize = _MAX_LFN;
...
)
{
	...
	mem_cpy(&dj_new, &dj_old, sizeof(FATDIR));
